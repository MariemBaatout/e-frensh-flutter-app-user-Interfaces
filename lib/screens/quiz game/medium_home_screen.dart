...
class MediumQuizListScreen extends StatefulWidget {
  const MediumQuizListScreen({Key? key}) : super(key: key);

  @override
  State<MediumQuizListScreen> createState() => _MediumQuizListScreenState();
}

class _MediumQuizListScreenState extends State<MediumQuizListScreen> {
  late String _token = '';
  List<Quiz> quizes = [];
  ...
  Future<void> getAllQuizes() async {
    ...
    if (response.statusCode == 200) {
      final responseData = json.decode(response.body) as List<dynamic>;
      final easyQuizzes =
          responseData.where((quiz) => quiz['type'] == 'medium').toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Liste Des Quiz Moyenne"),
        centerTitle: true,
        leading: IconButton(
