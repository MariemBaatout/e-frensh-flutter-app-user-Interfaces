...
class HardQuizListScreen extends StatefulWidget {
  const HardQuizListScreen({Key? key}) : super(key: key);

  @override
  State<HardQuizListScreen> createState() => _HardQuizListScreenState();
}

class _HardQuizListScreenState extends State<HardQuizListScreen> {
  late String _token = '';
  List<Quiz> quizes = [];
  ...
  Future<void> getToken() async {
    ...
    // After getting the token, fetch quizzes
    getAllQuizes();
  }
  ...
  Future<void> getAllQuizes() async {
    ...
    if (response.statusCode == 200) {
      final responseData = json.decode(response.body) as List<dynamic>;
      final easyQuizzes =
          responseData.where((quiz) => quiz['type'] == 'hard').toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Liste Des Quiz Difficile"),
        centerTitle: true,
        leading: IconButton(
